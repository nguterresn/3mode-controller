...
esp_now_peer_info_t peer_info = { .peer_addr = { 0x1A, 0xFF, 0x00, 0xFF, 0x00, 0xFF },
	                              .channel   = 0,
	                              .encrypt   = false };
...
{
	struct stamp stamp  = { .x = x, .y = y };
	esp_err_t    result = esp_now_send(peer_info.peer_addr, (uint8_t*)&stamp, sizeof(struct stamp));
